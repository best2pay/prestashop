...
class Best2PayValidationModuleFrontController extends ModuleFrontController {
	...
	public function postProcess() {
		...
		$best2pay_id = $this->module->registerOrder($order, $customer, $address, $currency);
		if(!$best2pay_id)
		...
		$args['signature'] = base64_encode(md5($sector_id . $best2pay_id . $shop_cart_encoded . $password));
		
		$url = $this->module->best2pay_url . $payment_path . "?" . urldecode(http_build_query($args));
